...
class ConfigurationService(IConfigurationService):
    ...
    def __init__(self, logger: Optional[ILogger] = None):
        ...
        try:
            ...
            if self.logger:
                self.logger.warning(f"Using user config directory: {self.user_config_dir}")
        
        if self.logger:
            self.logger.info(f"Configuration service initialized with base dir: {self.config_base_dir}")
                ...
                if config_path.exists():
                    try:
                        ...
                        if ssid and password:
                            if self.logger:
                                self.logger.info(f"Network config loaded from {config_path}")
                            return (ssid, password)
                        
                    except (json.JSONDecodeError, PermissionError) as e:
                        if self.logger:
                            self.logger.warning(f"Could not load from {config_path}: {e}")
                        continue
            ...
            for config_path in config_paths:
                if config_path.exists():
                    try:
                        config_path.unlink()
                        if self.logger:
                            self.logger.info(f"Network config cleared from {config_path}")
                        cleared = True
