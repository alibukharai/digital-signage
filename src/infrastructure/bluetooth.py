...
from ..interfaces import IBluetoothService, DeviceInfo, ILogger
...
class BluetoothService(IBluetoothService):
    ...
    def __init__(self, config: BLEConfig, logger: Optional[ILogger] = None):
        ...
        if not BLEAK_AVAILABLE:
            if self.logger:
                self.logger.warning("Bleak library not available, BLE functionality will be simulated")
    ...
    def start_advertising(self, device_info: DeviceInfo) -> bool:
        ...
        try:
            ...
            if not BLEAK_AVAILABLE:
                ...
                if self.logger:
                    self.logger.warning("BLE advertising simulated (Bleak not available)")
                return True
            ...
            if self.logger:
                self.logger.info(f"BLE advertising started with data: {advertising_data}")
    ...
    def _run_ble_server(self):
        ...
        try:
            ...
            while self.is_advertising_flag:
                ...
                if random.random() < 0.001 and self.credentials_callback:  # 0.1% chance per second
                    # Simulate received credentials
                    ssid = "TestNetwork"
                    password = "testpassword123"
                    
                    if self.logger:
                        self.logger.info(f"Simulated BLE credentials received: SSID={ssid}")
