...
from ..interfaces import (
    ...
    IHealthMonitor, ILogger, DeviceInfo, NetworkInfo, ConnectionInfo
)
...
from ..domain.errors import ProvisioningError, NetworkError, SecurityError
...
class NetworkProvisioningUseCase:
    ...
    def _on_credentials_received(self, ssid: str, password: str):
        ...
        try:
            ...
            is_valid, errors = self.validation_service.validate_wifi_credentials(ssid, password)
            if not is_valid:
                if self.logger:
            ...
            self.state_machine.process_event(ProvisioningEvent.CREDENTIALS_RECEIVED, {'ssid': ssid})
