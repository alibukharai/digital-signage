...
class ProvisioningStateMachine:
    ...
    def process_event(self, event: ProvisioningEvent, data: Any = None) -> bool:
        ...
        self.event_bus.publish(
            EventType.PROVISIONING_STARTED if self.current_state == DeviceState.PROVISIONING
            else EventType.PROVISIONING_COMPLETED if self.current_state == DeviceState.CONNECTED
            else EventType.PROVISIONING_FAILED if self.current_state == DeviceState.ERROR
    ...
    def reset(self) -> None:
        ...
        if self.logger:
            self.logger.info(f"State machine reset: {old_state.value} -> {self.current_state.value}")
